...
st.set_page_config(page_title="AirBnb-Analysis by Apoorva khare!!!", page_icon=":bar_chart:", layout="wide")
...
if SELECT == "Explore Data":
    ...
    if fl is not None:
        ...
        os.chdir(r"C:\Users\Dell\JupyterPythoncodes")
        df = pd.read_csv("Airbnb_data.csv")

st.sidebar.header("Choose your filter: ")

 # Create for neighbourhood_group
neighbourhood_group = st.sidebar.multiselect("Pick your neighbourhood_group", df["Host_neighbourhood"].unique())
if not neighbourhood_group:
     df2 = df.copy()
else:
     df2 = df[df["Host_neighbourhood"].isin(neighbourhood_group)]

 # Create for neighbourhood
neighbourhood = st.sidebar.multiselect("Pick the neighbourhood", df2["Host_neighbourhood"].unique())
if not neighbourhood:
     df3 = df2.copy()
else:
     df3 = df2[df2["Host_neighbourhood"].isin(neighbourhood)]
...
if not neighbourhood_group and not neighbourhood:
     ...
     filtered_df = df[df["Host_neighbourhood"].isin(neighbourhood_group)]
elif not neighbourhood_group:
     filtered_df = df[df["Host_neighbourhood"].isin(neighbourhood)]
elif neighbourhood:
     filtered_df = df3[df["Host_neighbourhood"].isin(neighbourhood)]
elif neighbourhood_group:
     filtered_df = df3[df["Host_neighbourhood"].isin(neighbourhood_group)]
elif neighbourhood_group and neighbourhood:
     filtered_df = df3[df["Host_neighbourhood"].isin(neighbourhood_group) & df3["Host_neighbourhood"].isin(neighbourhood)]
else:
     filtered_df = df3[df3["Host_neighbourhood"].isin(neighbourhood_group) & df3["Host_neighbourhood"].isin(neighbourhood)]
...
data1 = px.scatter(filtered_df, x="Host_neighbourhood", y="Host_neighbourhood", color="Room_type")
data1['layout'].update(title="Room_type in the Neighbourhood and Neighbourhood_Group wise data using Scatter Plot.",
                        titlefont=dict(size=20), xaxis=dict(title="Host_neighbourhood", titlefont=dict(size=20)),
                        yaxis=dict(title="Host_neighbourhood", titlefont=dict(size=20)))
...
with st.expander("Summary_Table"):
    df_sample = df[0:5][["Host_neighbourhood", "Host_neighbourhood", "Review_scores", "Room_type", "Price", "Min_nights", "Host_name"]]
